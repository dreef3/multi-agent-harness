...
describe("heartbeatMonitor", () => {
  ...
  it("fires injectMessage and broadcastStuckAgent after 4 minutes", async () => {
  ...
  it("does not fire if clearHeartbeat called first", async () => {
    const mockInject = vi.fn().mockResolvedValue(undefined);
    (getPlanningAgentManager as ReturnType<typeof vi.fn>).mockReturnValue({ injectMessage: mockInject });
  ...
  it("resetting before timeout cancels the previous timer", async () => {
    const mockInject = vi.fn().mockResolvedValue(undefined);
    (getPlanningAgentManager as ReturnType<typeof vi.fn>).mockReturnValue({ injectMessage: mockInject });
    ...
    await vi.advanceTimersByTimeAsync(3 * 60 * 1000); // total 6 min from first, only 3 from second

    expect(mockInject).not.toHaveBeenCalled();

    await vi.advanceTimersByTimeAsync(60 * 1000 + 100); // now 4+ min from second reset
    expect(mockInject).toHaveBeenCalledOnce();
  });
});
